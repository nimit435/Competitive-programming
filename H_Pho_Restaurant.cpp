...
void solve() {
    ...
    ll mx1 = 0;
    fl(i,n){
        mx0 = max(mx0, vec[i].ff);
        mx1 = max(mx1, vec[i].ss);
    ...
    vll mndiffo(n, 1e16);
    vll mndiffz(n, 1e16);
    mndiffo[0] = vec[0].ss-vec[0].ff; 
    mndiffz[0] = vec[0].ff-vec[0].ss;
    for(int i=1; i<n; i++){
       mndiffo[i] = min(mndiffo[i-1], vec[i].ss-vec[i].ff);
       mndiffz[i] = min(mndiffz[i-1], vec[i].ff-vec[i].ss);
    }
    vll dp(n, 1e16);
    ll sum = min(vec[0].ff, vec[0].ss)+ min(vec[1].ff, vec[1].ss);
    ll sumz = vec[0].ff+ vec[1].ff;
    ll sumo = vec[0].ss+ vec[1].ss;
    ll mnz = min(vec[0].ff, vec[1].ff);
    ll mno = min(vec[0].ss, vec[1].ss);
    if(sumz ==0 || sumo==0){
        dp[1] = 0;
    }
    else{
        dp[1] = min(vec[0].ff+vec[1].ss, vec[0].ss+ vec[1].ff);
    }

    // dp[1] = min(vec[0].ff+vec[1].ss, vec[0].ss+ vec[1].ff);
    for(int i=2; i<n; i++){
        if(sumz+vec[i].ff == 0 || sumo+vec[i].ss == 0){
            dp[i] = 0;
            sum += min(vec[i].ss, vec[i].ff);
            sumz += vec[i].ff;
            sumo += vec[i].ss;
            mnz = min(mnz, vec[i].ff);
            mno = min(mno, vec[i].ss);
            continue;
        }
       
        if(sumz == 0 && vec[i].ff!=0){
            dp[i] = min(vec[i].ss, vec[i].ff + mno);
            sum += min(vec[i].ss, vec[i].ff);
            sumz += vec[i].ff;
            sumo += vec[i].ss;
            mnz = min(mnz, vec[i].ff);
        ...
        if(sumo == 0 && vec[i].ss!=0){
            dp[i] = min(vec[i].ff, vec[i].ss + mnz);
            sum += min(vec[i].ss, vec[i].ff);
            sumz += vec[i].ff;
            sumo += vec[i].ss;
            mnz = min(mnz, vec[i].ff);
        ...
        dp[i] = dp[i-1] + min(vec[i].ss, vec[i].ff);

        // i -> primary one table
        if(mndiffo[i-1]<=0){
            dp[i] = min(dp[i], vec[i].ff+ sum);
        }
        else{
            dp[i] = min(dp[i], mndiffo[i-1]+sumz+vec[i].ff);
        }
        // i -> primary zero table
        if(mndiffz[i-1]<=0){
            dp[i] = min(dp[i], vec[i].ss+ sum);
        }
        else{
            dp[i] = min(dp[i], mndiffz[i-1]+sumo+vec[i].ss);
        }
        sum += min(vec[i].ss, vec[i].ff);
        sumz += vec[i].ff;
        sumo += vec[i].ss;
