...
//Code
void func(ll i, ll j, vector<vector<bool>>& vis, vector<string>& graph){
    vis[i][j] = true;
    if(i>0 && graph[i-1][j]=='.' && !vis[i-1][j]){
        func(i-1, j, vis, graph);
    }
    if(j>0 && graph[i][j-1]=='.' && !vis[i][j-1]){
        func(i, j-1, vis, graph);
    }
    if(i<graph.size()-1 && graph[i+1][j]=='.' && !vis[i+1][j]){
        func(i+1, j, vis, graph);
    }
    if(j<graph[0].size()-1 && graph[i][j+1]=='.' && !vis[i][j+1]){
        func(i, j+1, vis, graph);
    }
}
void solve() {
