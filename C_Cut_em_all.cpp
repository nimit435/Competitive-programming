...
//Code
ll getSize(ll i, ll par, vvll& graph, vll& sz){
    if(graph[i].size()==1 && par != -1){
        sz[i] = 1;
        return 1;
    }
    sz[i] = 1;
    for(auto it: graph[i]){
        ...
        if(it!= par){
            sz[i] += getSize(it, i , graph , sz);
        }
    }
    return sz[i];

}
ll Dfs(int i , int par, vvll& graph, vll& sz){
...
void solve() {
    ...
    vll sz(n+1, 0); 
    getSize(1, -1, graph, sz);
    // printvec(sz);
    cout<<Dfs(1, -1, graph, sz)<<endl;
}
// Allah hu Akbar
