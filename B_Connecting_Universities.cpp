...
//Code
ll func(ll i, ll par, vll& num, vvll& tree, vector<bool>& ispres, vll& parent){
    ll res = 0;
    if(ispres[i]){
        res++;
    }
    for(auto it: tree[i]){
        if(it != par){
            parent[it] = i;
            res += func(it, i, num, tree, ispres, parent);
        }
    }
    num[i] = res;
    return res;
...
void solve() {
    ...
    vll num(n);
    func(0, -1, num, tree, ispres, parent);
    ll res = 0;
    for(auto it: edges){
        ll a = it.ff;
        ll b = it.ss;
