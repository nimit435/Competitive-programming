...
//Code
ll getmanhattan(ll x, ll y, pll p){
    return (abs(x-p.ff)+abs(y-p.ss));
}
ll ask(ll x, char dir){
    cout<<"? "<<dir<<" ";
    cout<<x<<" "<<endl;
    ll res;
    cin>>res;
...
void solve() {
    ...
    ll mn = 1e16;
    fl(i,n){
        if(getmanhattan(-1e9, -1e9, anc[i])<mn){
            mn = getmanhattan(-1e9, -1e9, anc[i]);
            p = i;
        }
    }
    ll x = anc[p].ff;
