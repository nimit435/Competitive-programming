...
//Code
ll func(ll i, ll parity, vvll& chil, vvll dp){
    if(chil[i].size()==0){
        dp[i][parity] = 0;
        return 0;
    }
...
void dfs(ll i, ll par,  vvll& tree, vvll& chil){
    for(auto it: tree[i]){
        if(par==it){
            continue;
        }
...
void solve() {
    ...
    cin>>root;
    root--;
    dfs(root, -1, tree, chil);
    // printvec(chil);
    if(func(root, 0, chil, dp)==1){
        cout<<"Ron"<<endl;
        return;
    }
