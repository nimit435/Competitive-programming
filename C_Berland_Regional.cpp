...
//Code
ll get_help(ll k , vll& pref){
    ll left= 1;
    ll right = k;
    ll ans = 0;
    while(right<pref.size()){
...
void get_help_help(vll& pref , vll& res){
    for(int k=1; k<=pref.size()-1; k++){
        res[k-1]+=get_help(k,pref);
    }
}
void solve() {
    ll n;
    ...
    vvll peruni(n);
    fl(i,n){
        peruni[univ[i]].pb(strength[i]);
    }

    fl(i,n){
        if(peruni[i].size()!=0){
            sort(peruni[i].rbegin(), peruni[i].rend());
        }
    }
