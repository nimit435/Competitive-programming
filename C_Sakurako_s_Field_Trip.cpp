...
//Code
ll getDist(ll a, ll b, ll c, ll d){
    ll res =0;
    if(a==b){
        res++;
    }
...
void solve() {
    ...
    // printvec(vecb);
    for(int i=1; i<n/2; i++){
        if(getDist(veca[i-1], veca[i], vecb[i-1], vecb[i])>getDist(veca[i-1], vecb[i], vecb[i-1], veca[i])){
            ll temp= veca[i];
            veca[i]= vecb[i];
            vecb[i]= temp;
        }
    ...
    // if(n%2!=0){
    //     if(getDist(veca[-1], veca[i], vecb[i-1], vec b[i])>getDist(veca[i-1], vecb[i], vecb[i-1], veca[i])){
    //         ll temp= veca[i];
    //         veca[i]= vecb[i];
    //         vecb[i]= temp;
    //     }
