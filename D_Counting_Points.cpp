...
ll cntnumpts( ll prevrad, ll currrad, ll prevcen, ll currcen, bool flag){
    ll res=0;

    if(currcen-currrad>prevcen+prevrad || flag){    
        
        for(int i = currcen-currrad; i<=currcen+currrad; i++){
            res+= (1+(2*((isqrt((currrad*currrad)-((i-currcen)*(i-currcen)))))));
        }
        return res;
    }
    else{
        ...
        for (int i=currcen-currrad; i<=prevcen+prevrad; i++){
            // cout<<i<<": "<<( (1+(2*(floor(sqrt((currrad*currrad)-((i-currcen)*(i-currcen)))))))- ((1+(2*(floor(sqrt((prevrad*prevrad)-((i-prevcen)*(i-prevcen)))))))))<<" ";
            res+= max(2*((isqrt((currrad*currrad)-((i-currcen)*(i-currcen))))-(isqrt((prevrad*prevrad)-((i-prevcen)*(i-prevcen))))),0LL);
        }
        for(int i = prevcen+prevrad+1; i<=currcen+currrad; i++){
            // cout<<i<<": "<<(1+(2*(floor(sqrt((currrad*currrad)-((i-currcen)*(i-currcen)))))))<<" ";
            res+= (1+(2*((isqrt((currrad*currrad)-((i-currcen)*(i-currcen)))))));
        }
        // cout<<endl;
        return res;
...
void solve() {
    ...
    // printvec(vec);
    ll en= vec[0].ff+vec[0].ss;
    ll rad= vec[0].ss;
    ll cen= vec[0].ff;
    ll res=0;
    res+= cntnumpts(-1, rad, -1, cen, true);
    for (int i=1; i<n; i++){
        if(vec[i].ff+vec[i].ss>en){
            // cout<<i<<endl;
            res+= cntnumpts(rad, vec[i].ss, cen, vec[i].ff, false);
            rad= vec[i].ss;
            cen= vec[i].ff;
            en= rad+cen;
        }
