...
pair<ll,string> helper(vll& freq, ll k, ll n, string& str){
    ll num = n/k;
...
void solve() {
    ...
    pair<ll, string> fir = helper(freq, n, n, str);
    string st = fir.ss;
    ll mn = fir.ff;
    for(auto k: fin_k){
        pair<ll, string> re = helper(freq, k, n, str);
        if(re.ff<mn){
            st = re.ss;
            mn = re.ff;
        }
