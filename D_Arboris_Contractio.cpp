...
void func2(ll i, ll tar, ll par, vvll& tree, vector<ll>& path, vll& fin){
    path.pb(i);
    if(i==tar){
        fin = path;
        return;
...
void solve() {
    ...
    for(int i=1; i<=n; i++){
        ...
        mn = min(mn, leaves-res);

    }
    // printvec(dp);
    // fl(i,fin.size()){
    //     if(tree[fin[i]].size()==1){
    //         ll ans;
    //         if(i==0){
    //             ans = func3(fin[i],-1, fin[i+1], tree);
    //                 if(tree[fin[i+1]].size()==1){
    //                     ans++;
    //                 }
    //         }
    //         if(i==fin.size()-1){
    ...
    //         mn = min(mn, leaves-ans);
    //     }
    //     else{
    //         ll ans = func3(fin[i], fin[i-1], fin[i+1], tree);
    //         if(tree[fin[i-1]].size()==1){
    //             ans++;
    //         }
    //         if(tree[fin[i+1]].size()==1){
