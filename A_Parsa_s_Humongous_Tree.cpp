...
//Code
ll func(ll i, ll par, ll type, vvll& graph , vpll& vec , vvll& dp){
    ll res = 0;
    if(dp[i][type]!=-1){
        return dp[i][type];
    }
    if(type == 0){
        for(auto it: graph[i]){
            if(it!=par){
                res += max(abs(vec[it].ff - vec[i].ff) + func(it, i, 0, graph, vec, dp), abs(vec[it].ss - vec[i].ff) + func(it, i, 1, graph, vec, dp));      
            }
        }
        ...
        for(auto it: graph[i]){
            if(it!=par){
                res += max(abs(vec[it].ff - vec[i].ss) + func(it, i, 0, graph, vec, dp), abs(vec[it].ss - vec[i].ss) + func(it, i, 1, graph, vec, dp));      
...
void solve() {
    ...
    ll res = max(func(0,-1, 0, graph, vec , dp), func(0, -1, 1, graph, vec, dp));
    cout<<res<<endl;
}
// Allah hu Akbar
