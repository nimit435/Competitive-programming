...
ll func(ll i, ll k, string& temp, vll& dp){
    if(leftchil(i)>k){
        if(temp[i] =='?'){
            dp[i] = 2;
            // cout<<i<<":: "<<dp[i]<<endl;
    ...
    if(temp[i] == '1'){
        dp[i] = func(rightchil(i), k, temp, dp);
    }
    else if(temp[i] == '0'){
        dp[i] = func(leftchil(i), k, temp, dp);
    }
    else{
        dp[i] = func(rightchil(i), k, temp, dp) + func(leftchil(i), k, temp, dp);
    }
    // cout<<i<<": "<<dp[i]<<endl;
    return dp[i];
}
void solve() {
    ...
    fl(j,q){
        ...
        if(c == '1'){
            ...
            if(leftchil(i)<=k){
                dp[i] = dp[leftchil(i)] + dp[rightchil(i)];
            }
            else{
                dp[i] = 2;
            }
        ...
        while(curr!=0){
            if(temp[curr]=='0'){
                dp[curr] = dp[leftchil(curr)];
            }
            else if(temp[curr] == '1'){
                dp[curr] = dp[rightchil(curr)];
            }
            else{
                dp[curr] = dp[leftchil(curr)]+dp[rightchil(curr)];
            }
            curr = getpar(curr);
        }
        // cout<<temp<<endl;
