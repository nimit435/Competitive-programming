...
//Code
ll func(ll i, ll k, vpll& a, vvll& dp, vll& pref){
    if(k==0){
        dp[i][0] = pref[i];
        return pref[i];
    }
    ...
    if(i==1){
        dp[1][k] = a[0].ss*(a[1].ff);
        return a[0].ss*(a[1].ff);
    }
    if(dp[i][k]!=-1){
        return dp[i][k];
    }
    ...
    for(int j= 0; j<=k; j++){
        if(i-j-1>=0){
            mn = min(func(i-j-1, k-j, a, dp, pref)+(a[i].ff-a[i-j-1].ff)*a[i-j-1].ss, mn);
        }
    }
    dp[i][k] = mn;
    return dp[i][k];
...
void solve() {
    ...
    vll pref(n+1);
    for(int i=1; i<=n; i++){
        pref[i] = pref[i-1]+(a[i-1].ss*(a[i].ff-a[i-1].ff));
    }
    // printvec(pref);
    // if(k==0){
    //     cout<<pref[n]<<endl;
