...
//Code
void func(ll i, ll par, vpll& pars, vvpll& chil, vvpll& tree){
    for(auto it: tree[i]){
        if(it.ff!=par){
            chil[i].pb(it);
            func(it.ff, i, pars, chil, tree);
        }
        else{
            pars[i] = it;
        }   
...
void solve() {
    ...
    vpll pars(n+1);
    vvpll chil(n+1);
    func(1, -1, pars, chil, tree);
    vector<map<ll,ll>> wi(n+1);
    for(int i=1; i<=n; i++){
        for(auto it: chil[i]){
            wi[i][vec[it.ff-1]]+=it.ss;
        }
    }
    ll sum =0;
    for(int i=1; i<=n; i++){
        for(auto it: chil[i]){
            if(vec[i-1]!=vec[it.ff-1]){
                sum+= it.ss;
            }
        }
    }
//    printvec(chil);
    fl(i,q){
        ...
        if(b==curr){
            ...
            if(wi[a].find(curr)!=wi[a].end()){
                sum+= wi[a][curr];
            }
            if(wi[a].find(b)!=wi[a].end()){
                sum-= wi[a][b];
            }

            if(a!=1){
                if(vec[pars[a].ff-1]==curr){
                    sum+= pars[a].ss;
                }
                else if(vec[pars[a].ff-1]==b){ 
                    sum-= pars[a].ss;
                }
                wi[pars[a].ff][curr]-=pars[a].ss;
                if(wi[pars[a].ff][curr]==0){
                    wi[pars[a].ff].erase(curr);
                }
                wi[pars[a].ff][b]+=pars[a].ss;
