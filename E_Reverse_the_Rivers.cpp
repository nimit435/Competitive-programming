...
void solve() {
    ...
    ll q;
    cin>>q;
    vector<vll> cities(m, vector<ll>(n,0));
    ...
    for(int i=1; i<n; i++){
        fl(j,m){
            cities[j][i]= (cities[j][i-1]|cities[j][i]);
        }
    }
    // printvec(cities);
    fl(i,q){
        ...
        fl(j,m){
            ...
            cin>>r;
            if(fl && c=='>'){
                ll temp = getmin(cities[region], r);
                lint= max(lint, temp);
                // cout<< j<<": "<<lint<< " "<<rint<<endl;
                if(lint>rint || temp==-1){
                    fl= false;
                }
            }
                ...
                ll temp = getmax(cities[region], r);
                rint= min(rint, temp);
                // cout<< j<<": "<<lint<< " "<<rint<<endl;
