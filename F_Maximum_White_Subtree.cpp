...
//Code
ll func(ll i, ll par, vll& color, vvll& tree, vll& dp){
    if(color[i]==1){
        dp[i] = 1;
    }
    else{
    ...
    for(auto it: tree[i]){
        if(it!=par){
            dp[i] += max(0LL, func(it, i, color, tree, dp));
        }
    }
    return dp[i];
}
void func2(ll i, ll par, vll& color, vvll& tree, vll& dp, vll& ans){
    ll orig = dp[i];
    for(auto it: tree[i]){
        if(it!=par){
            ll ori = dp[it];
...
void solve() {
    ...
    vll dp(n);
    func(0, -1, color, tree, dp);
    vll ans(n);
    ans[0] = dp[0];
    func2(0, -1, color, tree, dp, ans);
    printvec(ans);
    // cout<<dp[0];
