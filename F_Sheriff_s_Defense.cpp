...
//Code
void Dfs(vector<vector<ll> >& dp, ll st, ll par, vector<vector<ll> > &tree, vector<ll> &coins, ll c){
    if(tree[st].size()==1 && tree[st][0]==par){
        dp[st][1]=coins[st];
        dp[st][0]= 0;
    }
    else{
        ...
        for(auto it: tree[st]){
            if(it!=par){
                dp[st][1] += max(dp[it][0], dp[it][1]- 2*c);
                dp[st][0] += max(dp[it][0], dp[it][1]);
            }
        }
    }
}
