...
//Code
void has_cycle(int i, int par, vector<set<ll>>& graph, vector<bool>& vis, bool &fl){
    vis[i] = true;
    for(auto it: graph[i]){
        if(vis[it] && it!=par){
            fl= true;
...
void solve() {
    ...
    fl(i,n){
        cin>>vec[i];
        graph[i+1].insert(vec[i]);
        graph[vec[i]].insert(i+1);
    }
    ...
    // cout<<"hig "<<cyc<<" "<<lin<<endl;
    if(lin==0){
        cout<<cyc<<" "<<cyc<<endl;
    }
    else{
        cout<<cyc+1<<" "<<cyc+lin<<endl;
    }
