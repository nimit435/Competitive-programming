...
//Code
bool compa(pair<pll, ll>& a, pair<pll, ll>& b){
    return a.ff.ff<b.ff.ff;
}
bool compb(pair<pll, ll>& a, pair<pll, ll>& b){
...
void solve() {
    ...
    fl(i, n-1){
        if(vec[i].ff.ff == vec[i+1].ff.ff){
            if(vec[i].ff.ss< vec[i+1].ff.ss){
                cout<<vec[i].ss<<" "<<vec[i+1].ss<<endl;
                return;
            }
            else{
                cout<<vec[i+1].ss<<" "<<vec[i].ss<<endl;
                return;
            }
        }
    }
    ...
    ll b = vec[0].ff.ss;
    for(int i= 1; i<n; i++){
        if(vec[i].ff.ff>a && vec[i].ff.ss<b){
            cout<<vec[i].ss<<" "<<vec[i-1].ss<<endl;
            return;
        }
        a = vec[i].ff.ff;
