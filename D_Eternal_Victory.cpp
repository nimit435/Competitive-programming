...
//Code
void dfs( ll i, ll par, vvpll& tree, vpll& parent, vvll& chil){
    for(auto it: tree[i]){
        if(it.ff!=par){
            chil[i].pb(it.ff);
            parent[it.ff] = mp(i, it.ss);
            dfs(it.ff, i, tree, parent , chil);
        }
    }
}
ll makesum(ll i, vvll& chil, vpll& parent, vll& sum){
    ll s = 0;
    for(auto it: chil[i]){
        s += parent[it].ss;
        s+= makesum(it, chil, parent, sum);
    }
    sum[i] = s;
    return s;
}
ll func(ll i, vvll& chil, vpll& parent, vll& sum, vll& dp){
    ...
    if(dp[i] != -1){
        ...
        ll ans = 1e16;
        for(auto it: chil[i]){
            ans = min(2*sum[i] - 2*sum[it] - parent[it].ss + func(it, chil, parent, sum , dp), ans);
        }
        dp[i] = ans;
        return ans;
    }
...
void solve() {
    ...
    vvll chil(n);
    vll sum(n);
    dfs(0, -1, tree, parent, chil);
    makesum(0, chil, parent, sum);
    vll dp(n, -1);
    // printvec(sum);
    cout<<func(0, chil, parent, sum, dp)<<endl;
