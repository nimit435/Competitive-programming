...
void solve() {
    ...
    vector<unordered_set<ll>> pr(mx+1);
    vector<unordered_set<ll>> edg(n);
    if(s==t){
        cout<<1<<endl;
        cout<<s<<endl;
        return;
    }
    if(gcd(vec[s-1],vec[t-1])>1){
        cout<<2<<endl;
        cout<<s<<" "<<t<<endl;
        return;
    }
