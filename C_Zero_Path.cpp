...
void solve() {
    ...
    vvll mindp(n, vll(m, -1));
    vvll maxdp(n, vll(m, -1));
    mindp[0][0] = grid[0][0];
    maxdp[0][0] = grid[0][0];
    for(int i= 1; i<m; i++){
    ...
    for(int i=1; i<n; i++){
        for(int j=1; j<m; j++){
            mindp[i][j] = grid[i][j] + min(mindp[i-1][j], mindp[i][j-1]);
            maxdp[i][j] = grid[i][j] + max(maxdp[i-1][j], maxdp[i][j-1]);
        }
    }
    // fl(i,n){
    //     printvec(mindp[i]);
    ...
    //     printvec(maxdp[i]);
    // }
    if((mindp[n-1][m-1]<0 && maxdp[n-1][m-1]<0) || (mindp[n-1][m-1]>0 && maxdp[n-1][m-1]>0)){
        cout<<"NO"<<endl;
    }
