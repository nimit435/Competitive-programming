...
//Code
void dijsktra(vector<vector<pair<pll, bool>>>& graph, vvll& dist){
    ...
    s.insert({0, 0});
    while(!s.empty()){
        auto [u , state] = *s.begin();
        s.erase(s.begin());
        for(auto it: graph[u]){
            ll v = it.ff.ff;
            ll w = it.ff.ss;
            bool is_forward = it.ss;
            if(state == 1){
                if(is_forward){
                    continue;
                }
...
void solve() {
    ...
    ll m;
    cin>>m;
    vector<vector<pair<pll, bool>>> graph(n);
    ...
    fl(i,m){
        ...
        cin>>u>>v>>w;
        u--; v--;
        graph[u].pb({mp(v, w), true});
        graph[v].pb({mp(u, w), false});


    }

    vvll dist(n, vll(2, 1e16));
    dijsktra(graph, dist);
    for(int i= 1; i<n; i++){
        if(min(dist[i][1], dist[i][0])== 1e16){
            cout<<-1<<" "; 
        }
        else{
            cout<<min(dist[i][1], dist[i][0])<<" ";
        }
    }
    cout<<endl;
