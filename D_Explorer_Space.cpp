...
//Code
ll func(ll i, ll j, ll k, vector<vvll>& dp, vvll& hor,  vvll& ver, ll n, ll m){
    if(k==0){
        return 0;
    }
    if(dp[i][j][k]!=-1){
    ...
    ll mn = 1e16;
    if(i>0){
        mn = min(mn, func(i-1, j, k-1, dp, hor, ver, n, m)+ver[i-1][j]);
    }
    if(i<n-1){
        mn = min(mn, func(i+1, j, k-1, dp, hor, ver, n, m)+ ver[i][j]);
    }
    if(j>0){
        mn = min(mn, func(i, j-1, k-1, dp, hor, ver, n, m)+ hor[i][j-1]);
    }
    if(j<m-1){
        mn = min(mn, func(i, j+1, k-1, dp, hor, ver, n, m)+ hor[i][j]);
    }
...
void solve() {
    ...
    vector<vvll> dp(n, vvll(m , vll((k/2)+1, -1)));
    fl(i, n){
        fl(j, m){
            cout<<2*func(i, j, k/2, dp, hor, ver, n , m)<<" ";
        }
        cout<<endl;
    }
}
