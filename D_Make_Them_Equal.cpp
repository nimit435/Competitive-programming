...
ll func2(ll i, ll k, vll& vec, vll& c,vll& dp, vvll& dp2){
    if(k<0){
        return 0;
    }
    if(i>=vec.size()){
    ...
    ll res = 0;
    if(k-dp[vec[i]]>=0){
        res = max(c[i]+func2(i+1, k-dp[vec[i]], vec, c, dp, dp2), func2(i+1 , k, vec, c, dp, dp2));
    }
    else{
        res = func2(i+1 , k, vec, c, dp, dp2);
    }
    dp2[i][k] = res;
    return res;
...
void solve() {
    ...
    vvll dp2(n+10, vll(k+10, -1));
    ll res = func2(0, k, vec, c, dp, dp2);
    cout<<res<<endl;
