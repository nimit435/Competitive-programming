...
//Code
ll func(ll i, vvll&chil, vll& height){
    ll res = 1;
    for(auto it: chil[i]){
        res = max(res, 1+func(it, chil, height));
    }
    height[i] = res;
    return res;
}
ll value(ll i, vvll& chil, vll& height){
    ll res = 1;
    ll temp = 0;
    for(auto it: chil[i]){
        temp += value(it, chil ,height);
    }
    res = max(height[i], temp);
    return res;
}
void solve() {
    ll n;
