...
//Code
ll find_ind(vvll& indices, ll curr,  int j){
    ll left = 0; // indices[c-'a'][left] < curr
    ll right = indices[j].size()-1; // indices[c-'a'][right] >= curr
    if(indices[j].size()==0){
        return -1;
    }
    if(indices[j][left] >= curr){
        return indices[j][left];
    }
    if(indices[j][right]<curr){
        return -1;
    }
    // cout<< left<<" "<<right<<endl;
    while(right-left>1){
        ll mid = (right+left)/2;
        if(indices[j][mid]<curr){
            left = mid;
        }
        else{
            right = mid;
