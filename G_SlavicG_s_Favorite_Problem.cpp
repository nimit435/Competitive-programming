...
//Code
void func(ll i, ll par, vll& pref, vvpll& tree){
    for(auto it: tree[i]){
        if(it.ff!=par){
            ll v = it.ff;
            ll w = it.ss;
...
void func2(ll i, ll par, vll& pref, vvpll& tree, bool fl, ll b){
    if(fl){
        pref[i] = -1;
    }
    for(auto it: tree[i]){
        if(it.ff!=par){
            if(!fl && it.ff==b){
                func2(it.ff, i, pref, tree , true, b);
            }
            else if(fl){
                func2(it.ff, i, pref, tree, true, b);
            }
            else{
                func2(it.ff, i, pref, tree, false, b);
            }
