...
//Code
ll func(ll i, ll par, vvll& tree, vector<bool>& task, vll& chil){
    ll res = 0;
    for(auto it: tree[i]){
        if(it!= par){
            if(task[it]){
            ...
            res += func(it, i, tree, task, chil);
        }
    }
    chil[i] = res;
    return res;
}
ll fin(ll i, ll par, vvll& tree, vll& chil, vector<bool>& task){
    ll res = 0;
    for(auto it: tree[i]){
        if(it == par){
            continue;
...
void compute(ll i, ll par, vvll& tree, vll& depth){
    for(auto it: tree[i]){
        if(it!=par){
            depth[it] = depth[i]+1;
            compute(it, i, tree, depth);
        }
    }
}
void solve() {
    ...
    vll chil(n, 0);
    func(x, -1, tree, task, chil);
    ll res = fin(x, -1, tree, chil, task);
    vll depth1(n, 0);
    compute(x, -1, tree, depth1);
    vll depth2(n, 0);

    compute(y, -1, tree, depth2);
    // printvec(depth1);

    // printvec(depth2);
    ll mn = res - depth1[tasks[0]] + depth2[tasks[0]];
    // printvec(tasks);
    for(int i = 1; i<k; i++){
        mn = min(mn, res-depth1[tasks[i]]+depth2[tasks[i]]);
    }
    cout<<mn<<endl;
}
// Allah hu Akbar
