...
//Code
ll func(ll i, ll par, vvll& tree, vvll& chil, vll& chilcnt){
    ll res = 1;
    for(auto it: tree[i]){
        if(it!=par){
            chil[i].pb(it);
            res += func(it, i, tree, chil, chilcnt);
        }
    }
    chilcnt[i] = res;
    return res;
}
void func2(ll i , vvll& chil, vll& chilcnt, vpll& res, bool& fl){
    if((chilcnt[i]%3)!=0){
        fl = false;
        return;
    }
    ...
    if(num>=3){
        ...
        ll it2 = -1;
        for(auto it: chil[i]){
            if(((chilcnt[it]%3)!=0) && (it1 == -1)){
                it1 = it;
            }
            else if((chilcnt[it]%3)!=0){
                it2 = it;
            }
            else{
                res.pb(mp(i, it));
                func2(it, chil, chilcnt, res, fl);
                if(!fl){
                    return;
                }
            }
        }
        for(auto it: chil[it1]){
            res.pb(mp(it1, it));
            func2(it, chil, chilcnt, res, fl);
            if(!fl){
                return;
...
void solve() {
    ...
    vvll chil(n);
    vll chilcnt(n,0);
    func(0 , -1, tree, chil, chilcnt);
    vpll res;
    bool fl = true;
    func2(0, chil ,chilcnt, res, fl);
    if(!fl){
        cout<<-1<<endl;
    }
    else{
