...
//Code
lld getRes(vll& vec, vll& t, ll x0){
    ll res = 0.0;
    fl(i,vec.size()){
        res = max(res,t[i]+abs(vec[i]-x0));
    }
    return res;
}
void solve() {
    ...
    while(r-l>1){
        ...
        ll a = mid - 1;
        ll b = mid + 1;
        ll t1 = getRes(vec, t, mid);
        ll t2 = getRes(vec, t, a);
        ll t3 = getRes(vec, t, b);
        // cout<<t1<<" "<<t2<<" "<<t3<<endl;
        if(t1<t2 && t1>t3){
            l = mid;
        }
    ...
    ll k = r;
    if(getRes(vec, t, l)<getRes(vec, t, r)){
