...
int main() {
    ...
    helper[1] = getinv(4);
    for(int i=2; i<=n; i++){
        ll a = ceil((((double)(i+1))/2));
        helper[i] = (helper[i-1]+((a*getinv(powermod(2,i+1, MOD)))%MOD))%MOD;
    }

    // for(int i= 2; i<n; i++){
    //     dp[i+1] = (powermod(2, i-1, MOD))%MOD; 
    //     // cout<<dp[i+1]<<endl;
    //     dp[i+1] = (dp[i+1]+sum)%MOD;
    //     // cout<<dp[i+1]<<endl;
    //     dp[i+1] = (dp[i+1]+ (((helper[i-1])*powermod(2,i,MOD))%MOD))%MOD;
    //     dp[i+1] = dp[i+1]+ (ceil((double)(i+1)/2));
    //     sum = (sum+dp[i+1])%MOD;
    // }
