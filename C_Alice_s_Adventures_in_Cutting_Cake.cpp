...
//Code
bool get_creat(ll i, ll j, ll m, vll & pref, vll& suff){
    if(i==0 && j == pref.size()-1){
        return false;
    }
    else if(i==0){
        if(suff[j+1]>=m){
...
ll getmx(ll i, ll m, vll& pref, vll& suff){
    if(get_creat(i, pref.size()-1, m, pref, suff)){
        return pref.size()-1;
    }
    else if(!get_creat(i,i,m,pref,suff)){
        return -1;
    }
    else{
        ll left= i; //get[left] = true;
        ll right = pref.size()-1; // get[right]= false;
        while(right-left>1){
            ll mid = (right+left)/2;
            if(get_creat(i, mid, m, pref, suff)){
                left = mid;
            }
            else{
                right = mid;
...
void solve() {
    ...
    psum[0] = cake[0];
    for(int i=1; i<n; i++){
        psum[i] = psum[i-1]+cake[i];
    }
    ll mx = 0;
    fl(i,n){
        ll j = getmx(i,m,pref,suff);
        if(i==0){
            if(j!=-1){
                mx = max(mx, psum[j]);
            }
