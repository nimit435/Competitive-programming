...
//Code
pair<ll,ll> get_cord(pll cord, ll dir){
    if(dir==0){
        return mp(cord.ff-1, cord.ss);
    }
    else if (dir==1){
        return mp(cord.ff+1, cord.ss);
    }
    else if (dir == 2){
        return mp(cord.ff, cord.ss+1);
    }
    else if(dir==3){
        return mp(cord.ff , cord.ss-1);
    }
...
void solve() {
    ...
    // cout<<stx<<" "<<enx<<endl;
    vector<vector<vector<vector<ll>>>> dp(n, vector<vector<vector<ll>>>(m, vector<vector<ll>>(4, vector<ll>(4, 1e16))));
    // ll res = func(dp, graph, stx,enx, n, m, 0, 0);
    ll min_cost = 1e16;
    priority_queue<tuple<ll, ll, ll, ll, ll>> pq; // cost i j dir state
    fl(d, 4){
        pll cord = get_cord(mp(stx, enx), d);
        if(cord.ff>=0 && cord.ff<=n-1 && cord.ss>=0 && cord.ss<=m-1 && graph[cord.ff][cord.ss]!='#'){
            ll cost = 1;
            ll state = 1;
            if (dp[cord.ff][cord.ss][d][state] > cost) {
                dp[cord.ff][cord.ss][d][state] = cost;
                pq.push({-cost, cord.ff, cord.ss, d, state});
            }
        }
    }
    while(!pq.empty()){
        auto [cost, r, c, dir, state] = pq.top();
        pq.pop();
        cost = -cost;
        if(cost>dp[r][c][dir][state]){
            continue;
        }
        if(graph[r][c] == 'T'){
        ...
        fl(d,4){
            pll cord = get_cord(mp(r,c), d);
            if(cord.ff>=0 && cord.ff<=n-1 && cord.ss>=0 && cord.ss<=m-1 && graph[cord.ff][cord.ss]!='#'){
                ll new_cost = cost+1;
                if(d==dir){
                    if(state<3){
                        if(dp[cord.ff][cord.ss][d][state+1]>new_cost){
                            dp[cord.ff][cord.ss][d][state+1]=new_cost;
                            pq.push({-new_cost, cord.ff, cord.ss, d, state+1});

                        }
                    }
                }
                else{
                    if(dp[cord.ff][cord.ss][d][1]>new_cost){
                        dp[cord.ff][cord.ss][d][1]=new_cost;
                        pq.push({-new_cost, cord.ff, cord.ss, d, 1});
                    }
                }
            }
        }
