...
//Code
void func(ll i, ll par,  vvll& tree, vvll& chil , vll& parent){
    for(auto it: tree[i]){
        if(it!=par){
            parent[it] = i;
            chil[i].pb(it);
            func(it, i, tree, chil, parent);
        }
    }
}
ll func2(ll i, ll state, vvll& chil, vvll& dp){
    if(chil[i].size()==0){
        return 0;
    }
    if(dp[i][state]!=-1){
    ...
    if(state==1){
        ...
        ll mx = sum;
        for(auto it: chil[i]){
            mx = max(mx, 1+func2(it, 1,  chil, dp)+sum - func2(it, 0, chil, dp));
        }
        dp[i][0] = mx;
        return mx;
    }
...
void solve() {
    ...
    vvll chil(n);
    vll parent(n, -1);
    func(0, -1, tree, chil, parent);
    vvll dp(n, vll(2,-1));
    cout<<func2(0, 0, chil, dp);
