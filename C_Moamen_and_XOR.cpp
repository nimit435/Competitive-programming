...
//Code
ll getnCr(ll n, ll r, vll& fact, ll MOD){
    if(r==0){
        return 1;
    }
    if(r==1){
    ...
    ll den_inv = powermod(den, MOD-2, MOD);
    ll res = (num*den_inv)%MOD;
    return res;
}
void solve() {
    ll n;
    ...
    if(k==0){
        ...
        for(int i= n-1; i>=2; i--){
            if(i%2==0){
                cnt = (cnt+getnCr(n,i,fact, MOD))%MOD;
            }
        }
        ll res = (powermod(cnt, k, MOD));
        cout<<res<<endl;
    }
    else{
        ll b = 1;
        ...
        for(int i= n-1; i>=2; i--){
            if(i%2==0){
                b = (b+getnCr(n,i,fact, MOD))%MOD;
            }
        }
        ll m = k-1;
        ll a = powermod(2, n, MOD);
        for(int i=0; i<=m; i++){
            res = (res+ ((powermod(a,m-i,MOD)*powermod(b,i,MOD))%MOD))%MOD;
        }
        res = (res+ powermod(b, k, MOD))%MOD;
        cout<<res<<endl;
    }
}
// Allah hu Akbar
