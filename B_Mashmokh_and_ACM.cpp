...
//Code
ll func(ll i, ll k , ll n, vvll& dp){
    if(dp[i][k]!=-1){
        return dp[i][k];
    }
    if(k==0){
    ...
    dp[i][k]= func(i,k-1,n,dp)%1000000007;
    for(int j= 2*i; j<=n; j+=i){
        dp[i][k] = (dp[i][k]+  (func(j, k-1, n , dp)%1000000007))%1000000007;
    }
    return dp[i][k];
}
void solve() {
    ...
    ll k;
    cin>> k;
    vvll dp(n+12, vector<ll>(k+12, -1));
    ll ans= 0;
    for(int i=1; i<=n; i++){
        ans = (ans+func(i,k,n,dp))%1000000007;
    }
    cout<<ans<<endl;
