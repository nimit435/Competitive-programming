...
//Code
bool bfs(int i,vector<vector<ll> >& graph, bool vis[], bool par[]){
    queue<int> q;
    q.push(i);
    vis[i]=true;
    bool flag= false;
...
bool isTwoColourable(vector<vector<ll> >& graph) {
    bool par[graph.size()];
    bool vis[graph.size()];
    memset(vis, false, sizeof(vis));
    for(int i=0; i<graph.size(); i++){
        if(!vis[i]){
            if(!bfs(i,graph, vis,par)){
                return false;
            }
        }
    }
