...
bool isSame(int i, int j, vll& parent){
    if(par(i,parent)== par(j,parent) ){
        return true;
    }
    return false;
}
void uni(int i, int j, vll& sz, vll& parent){
    int a= par(i, parent);
    int b= par(j, parent);
    if(sz[a]>sz[b]){
        swap(a,b);
...
void solve() {
    ...
    fl(i,n){
        if(!isSame(i,vec[i], parent)){
            uni(i,vec[i], sz, parent);
        }
    }
    ll res=0;
    fl(i,n){
