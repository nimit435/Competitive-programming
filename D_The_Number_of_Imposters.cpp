...
//Code
void fill_arr(vvpll& graph, ll i, ll role, vll& roles , vll& fin, bool& fl){
    fin.pb(i);
    for(auto it: graph[i]){
        if(roles[it.ff]==-1){
            if(role == 1){
            ...
            fill_arr(graph, it.ff, roles[it.ff], roles, fin, fl);
        }
        else{
            ll act;
            if(role == 1){
...
void solve() {
    ...
    fl(i,m){
        ...
        if(flag){
            if(st[0]=='i'){
                // if(hm.find({a,b,0})!=hm.end() || hm.find({b,a,0})!=hm.end()){
                //     flag = false;
                // }
                // else{
                    graph[a].pb(mp(b,1));
                ...
                // if(hm.find({a,b,1})!=hm.end() || hm.find({b,a,1})!=hm.end()){
                //     flag = false;
                // }
                // else{
                    graph[a].pb(mp(b,0));
    ...
    for(int i=1; i<=n; i++){
        if(roles[i]==-1){
            ...
            bool fl2 = true;
            roles[i]= 0;
            fill_arr(graph, i, 0, roles, fin, fl);
            if(!fl){
                cout<<-1<<endl;
                return;
            }
