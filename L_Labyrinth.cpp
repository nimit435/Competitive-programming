...
//Code
void dfs(ll i, vvll& graph, vector<bool>& vis, vll& parent, vvll& chil){
    vis[i] = true;
    for(auto it: graph[i]){
        if(!vis[it]){
            parent[it] = i;
            chil[i].pb(it);
            dfs(it, graph, vis, parent, chil);
        }
    }
}
...
void solve() {
    ...
    auto issame = [&](ll a, ll b){
        return find(a)==find(b);
    };
    auto unionfind = [&](ll a, ll b){
        a = find(a);
        b = find(b);
        if(size[a]>size[b]){
            swap(a,b);
