...
//Code
ll func(ll n, vpll& vec, map<pll, ll>& hm, vll& dp){
    ...
    if(dp[n]!=-1){
        ...
        ll temp = vec[n].ss;
        if(temp==1){
            dp[n] = (n*n)+func(hm[mp(curr-1, 1)], vec, hm, dp);
            return dp[n];
        }
        else if(temp == curr){
            dp[n] = (n*n)+ func(hm[mp(curr-1, curr-1)], vec, hm, dp);
            return dp[n];
        }
        else{
            dp[n] = n*n;
            dp[n] += func(hm[mp(curr-1, temp-1)], vec, hm, dp);
            dp[n] += func(hm[mp(curr-1, temp)], vec, hm, dp);
            dp[n] -= func(hm[mp(curr-2, temp-1)], vec, hm, dp);
