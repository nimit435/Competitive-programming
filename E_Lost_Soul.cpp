...
void solve(bool fl) {
    ...
    ll res = -1;
    for(auto it: hm){
        sort(it.ss.begin(), it.ss.end(), comp);
        for(int i = it.ss.size()-2; i>=0; i--){
            if((it.ss[i].ff!=(it.ss[i+1].ff-1)) ){
                res = max(res, it.ss[i].ff);
                break;
            }
            else{
                if((it.ss[i].ss==it.ss[i+1].ss)){
                    res= max(res, it.ss[i].ff);
                    break;
                }
                else{
                    if(i>=1){
                        res = max(res, it.ss[i-1].ff);
                        break;
                    }
                }
            }
