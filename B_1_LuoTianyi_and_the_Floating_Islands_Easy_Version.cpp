...
//Code
void dfs(ll i, ll par, vvll& tree, vll& sz, vvll& chil){
    sz[i] = 1;
    for(auto it: tree[i]){
        if(it!= par){
            chil[i].pb(it);
            dfs(it, i, tree, sz, chil);
            sz[i] += sz[it];
        }
    }
    
}
ll prob(ll num, ll den, ll MOD){
    ll deninv = powermod(den, MOD-2, MOD);
    ll res = (num*deninv)%MOD;
    return res;
}
void solve() {
    ll n;
    ...
    if(k==2){
        ...
        for(int i=1; i<=n; i++){
            ll temp = 0;
            for(auto it: chil[i]){
                temp += (sz[it])*(sz[i]-sz[it]-1);
            }
            temp = temp/2;
            temp += (n-1);
            temp += (n-sz[i])*(sz[i]-1);
            res = (res + prob((temp%MOD), (((n*(n-1))/2)%MOD), MOD))%MOD;
        }
        cout<<res<<endl;
        return;
    }
    if(k==3){
        ...
        for(int i=1; i<=n; i++){
            ...
            for(auto it: chil[i]){
                // cout<<it<<": ";
                ll rest = temp - (sz[it]*(sz[i]-sz[it]-1));
                // cout<<rest<<" "<<rest*sz[it]<<" ";
                fin += rest*sz[it];
            }
            // cout<<endl;
            // cout<<fin<<endl;
            ...
            // cout<<i<<": "<<fin<<" "<<temp<<endl;
            res =  (res + prob((fin%MOD), (((n*(n-1)*(n-2))/6)%MOD), MOD))%MOD;
