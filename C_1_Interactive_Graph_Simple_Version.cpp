...
void solve() {
    ...
    auto fillinfo = [&](ll x, ll st){
        ll left = st; // k[left][0] = x;
        ll right = r;
        while(right-left>1){
        ...
        chilinfo[x-1] = {st, left-st+1};
    };
    ll start= 1;
    for(int i=1; i<=n; i++){
        fillinfo(i, start);
        start = chilinfo[i-1].ff + chilinfo[i-1].ss;
    }
    ...
    vpll edges;
    for(int i=1; i<=n; i++){
        ll ri = chilinfo[i-1].ff+chilinfo[i-1].ss-1;
        while(lastn[i-1]!=-1){
            edges.pb({i, lastn[i-1]});
            ll lst = lastn[i-1];
            ri = ri- chilinfo[lst-1].ss;
            if(ri>chilinfo[i-1].ff){
                vll vec = ask(ri);
                lastn[i-1] = vec[1];
            }
            else{
