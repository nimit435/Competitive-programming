...
ll min_diff_v2(ll num, vll& vec){
    if(vec[vec.size()-1]<=num){
        return num-vec[vec.size()-1];
    }
    else if(vec[0]>=num){
        return vec[0]-num;
    }
    ll low = 0; //vec[low]< num
    ll high = vec.size()-1; //vec[high]>=num
    while(high-low>1){
    ...
    ll mn = min(vec[high]-num, num-vec[low]);
    return mn;
}
void solve() {
    ll n;
    ...
    mn = vec[0];
    if(k==1){
        mn = min(min_diff(vec), mn);
        cout<<mn<<endl;
        return;
    }
    if(k==2){
        mn = min(mn, min_diff(vec));
        fl(i,n-1){
            for(int j=1; j<n; j++){
                mn = min(mn, min_diff_v2(vec[j]-vec[i], vec));
            }
        }
        cout<<mn<<endl;
        return;
