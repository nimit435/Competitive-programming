...
void solve() {
    ...
    if(zer.empty()){
        ...
        for(auto ind: zer){
            if(ind>left){
                pll t = getMx(vec, left, ind-1);
                ll tem = 0;
                for(int i = t.ff; i<=n-t.ss-1; i++){
                    if(abs(vec[i])==2){
                        tem++;
                    }
                }
        ...
        if(n>left){
            pll t = getMx(vec, left, n-1);
            // cout<<"uh"<<" "<<t.ff<<" "<<t.ss<<endl;
            ll tem = 0;
            for(int i = t.ff; i<=n-t.ss-1; i++){
                if(abs(vec[i])==2){
                    tem++;
