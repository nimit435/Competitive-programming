...
//Code
ll func(ll i, vll& par, vvll& tree , vll& dp, map<pll, ll>& hm){
    if(par[i]==0){
        dp[i]=1;
        return 1;
    }
    ...
    if(hm[mp(i, par[i])]>hm[mp(par[i], par[par[i]])]){
        dp[i] = func(par[i], par, tree, dp, hm);
        return dp[i];
    }
    else{
        dp[i] = func(par[i], par, tree, dp, hm)+1;
        return dp[i];
    }

    
}
void assign_par(ll i , ll pa, vvll& tree , vll& par){
    if(tree[i].size()==1 && par[i]!=-1){
        return;
    }
    else{
        for(auto it: tree[i]){
...
void solve() {
    ...
    vll par(n);
    par[0] = -1;
    assign_par(0, -1, graph, par);
    
    vll dp(n, -1);
    dp[0]=0;
    for(int i=1; i<n; i++){
        func(i, par, graph, dp, hm);
    }
    ll mx =0;
