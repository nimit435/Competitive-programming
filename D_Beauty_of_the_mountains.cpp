...
void solve() {
    ...
    vvll prefa(n+1, vll(m+1));
    // vvll prefb(n+1, vll(m+1));

    for(int i=1; i<=n; i++){
        for(int j= 1; j<=m; j++){
            if(vec[i-1][j-1] == '0'){
                prefa[i][j] = prefa[i-1][j] +1;
            }
            else{
    ...
    for(int i=0; i<=n-k; i++){
        ll ki = 0;
        fl(j, k){
            ki += prefa[i+k][j+1]- prefa[i][j+1];
        }
        coeff.pb(abs((2*ki)-(k*k)));
        for(int j = 1; j<= m-k; j++){
            ki -= prefa[i+k][j]-prefa[i][j];
            ki += prefa[i+k][j+k] - prefa[i][j+k]; 
            coeff.pb(abs((2*ki)-(k*k)));
        }
    }
    // printvec(coeff);
