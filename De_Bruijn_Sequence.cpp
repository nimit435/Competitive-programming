...
//Code
void DFS(ll i, vector<bool>& seen, vll& path, vvpll& graph){
    while(!graph[i].empty()){
        auto [child, id] = graph[i].back();
        graph[i].pop_back();
        if(seen[id]){
            continue;
        }
        seen[id] = true;
        DFS(child, seen, path, graph);
    }
    path.pb(i);
}
void solve() {
    ...
    // graph[0].pb(mp(pow(2, n-1)-1, e));
    // e++;
    vector<bool> seen(e);
    vll path;
    DFS(0, seen, path, graph);
    // printvec(path);
    reverse(vr(path));
    // printvec(path);

    for(int i=1; i<path.size(); i++){
        if(hmb[path[i]].back() == '1'){
            st+='1';
        }
        else{
            st+='0';
